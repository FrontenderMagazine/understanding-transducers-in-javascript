...
Последняя визуализация показывает, что мы хотим получить композицию функций map, filter и reduce и передавать данных последовательно каждой из них. Вместо привычного:
...
Нам нужно будет написать что то вроде:
...
Давайте снова взглянем на наши редьюсеры. Они оба используют `Array.prototype.push()` в качестве
*сокращающей функции*:
